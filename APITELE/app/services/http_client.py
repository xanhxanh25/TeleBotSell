...
class HttpClient:
    def __init__(
        ...
        # total: end-to-end; sock_read=None — tránh sock_read=3s cắt response backend chậm dưới tải (gây retry storm).
    ...
    async def close(self) -> None:
        # ClientSession.close() đóng luôn TCPConnector khi session là owner
    ...
    async def request(self, method: str, url: str, json: dict | None = None, headers: dict | None = None, params: dict | None = None) -> Any:
        ...
        max_attempts = self.retries if idempotent else 1
        last_err = None
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if attempt == max_attempts:
                    log.warning("HTTP %s %s attempt=%s/%s err=%s", method_u, url, attempt, max_attempts, last_err)
                if attempt < max_attempts:
                ...
                if msg.startswith("http_error:4"):
                    raise
                if attempt == max_attempts:
                    log.warning("HTTP %s %s attempt=%s/%s err=%s", method_u, url, attempt, max_attempts, e)
                if attempt < max_attempts:
                    delay = min(0.2 * (2 ** (attempt - 1)), 1.0)
                    await asyncio.sleep(delay)
