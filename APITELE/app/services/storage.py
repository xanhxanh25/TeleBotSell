...
class UserSession:
    ...
    menu_product_ids: Optional[list[str]] = None
    ...
    error_photo_file_id: Optional[str] = None  # Ảnh đính kèm bảo hành
...
class MemoryStorage:
    ...
    def reset_flow(self, tid: int):
        ...
        with user_lock:
            ...
            s.menu_product_ids = None
            s.topup_network = None
            s.topup_coin = None
            s.history_month = None
            s.history_year = None
            s.history_page = None
    ...
    def stop_cleanup_task(self):
        """Dừng cleanup task (sync cancel — dùng stop_cleanup_task_async trong shutdown async)."""
        if self._cleanup_task and not self._cleanup_task.done():
            self._cleanup_task.cancel()

    async def stop_cleanup_task_async(self) -> None:
        """Hủy và await cleanup task (tránh task leak khi shutdown)."""
